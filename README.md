...
- 通过养鸡宝微信二维码登录，读取多账户列表、基金持仓、账户汇总和收益曲线。
- 使用新浪基金、东方财富净值和证券行情补充实时估值；公开估值缺失时，按联接 ETF、跟踪指数或已披露股票持仓逐级推导。
- 根据持有金额、份额和实时估值重新计算基金及账户的当日收益与收益率。
- 展示上证指数、深证成指、创业板指、沪深 300、上证 50 等指数行情，并支持浏览器角标显示当日收益或收益率。
- 支持基金搜索、添加持仓、删除持仓和持仓排序。
- 将 App 自选基金的全部分组及基金列表缓存到浏览器；日常打开、切换分组和刷新不访问 App 登录接口。
- 自选列表展示估算净值、当日涨幅和关联板块；列表变更通过“同步列表”按钮手动更新，基金行情仍可独立刷新。
- 点击持仓、自选或搜索结果中的基金打开详情浮层，查看历史净值和盘中净值估算走势。
- 支持浅色与暗黑主题切换，并在下次打开插件时保留主题设置。
